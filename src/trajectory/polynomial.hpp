...
template<int Exp>
static inline constexpr double const_powi(double base)
{
    static_assert(Exp >= -2 && Exp <= 5, "invalid exponent: must be between 0 and 5");
...
static inline constexpr double inner_eval(double coeff, double t)
{
    static_assert(Order >= 0 && Order <= 2, "invalid order: must be 0, 1 or 2");

    switch (Order) {
    case 2:
        return coeff * (Index - 1.0) * Index * const_powi<Index - 2>(t);
    case 1:
        return coeff * Index * const_powi<Index - 1>(t);
    case 0:
        return coeff * const_powi<Index>(t);
    default:
       std::abort();
    }
}
...
template<int Degree, int X0, int XD>
template<int Order>
inline constexpr double PolynomialTrajectory<Degree, X0, XD>::const_eval(double t) const
{
    static_assert(Order >= 0 && Order <= 2, "invalid Order: must be 0, 1 or 2");
    static_assert(Degree == 4 || Degree == 5, "invalid Degree: must be 4 or 5");

    double res {0};

    if (Order < 1) {
        res += inner_eval<Order, 0>(coeffs[0], t);
    }
    if (Order < 2) {
        res += inner_eval<Order, 1>(coeffs[1], t);
    }
    res += inner_eval<Order, 2>(coeffs[2], t);
    res += inner_eval<Order, 3>(coeffs[3], t);
    if (Degree == 5) {
        res += inner_eval<Order, 4>(coeffs[4], t);
    }
